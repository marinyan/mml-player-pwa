...
| `<` / `>` | オクターブ上下 |
| `V<number>` | 音量 0-15 |
| `cresc.` / `dim.` | 発音イベントごとに音量を1段階ずつ増加 / 減少。`V<number>` で解除 |
...
`cresc.` と `dim.` は以降の音符ごとに音量を1段階ずつ変化させます。和音は
和音全体で1段階として扱い、休符では進行しません。明示的な `V<number>` を
指定すると解除され、その音量へ戻ります。音量は0〜15の範囲でクランプします。
